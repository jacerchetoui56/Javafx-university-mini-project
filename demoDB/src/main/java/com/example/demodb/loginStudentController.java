...
public class loginStudentController implements Initializable {
    ...
    public void login(ActionEvent event) throws IOException {
        ...
        String query=" Select * from liste where firstName=?";
        try {
            String prenom = "";
            String nom ="";
            String cin ="";
            ...
            while (rs.next()) {
                ...
                group = rs.getString("classe");
                id= rs.getString("id");
                count++;
            }
            if (count > 0 && password.getText().equals("admin")){
                profilStudentController.infos = new String[]{prenom + " " + nom,group,cin,id};
    ...
    public void signup(ActionEvent event) throws IOException{
        FXMLLoader loader = new FXMLLoader(getClass().getResource("signUpStudent.fxml"));
        root = loader.load();
        // root = FXMLLoader.load(getClass().getResource("hellofx.fxml"));
        stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
        scene = new Scene(root);
